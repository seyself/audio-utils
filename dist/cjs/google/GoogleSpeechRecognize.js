...
exports.speechRecognize = speechRecognize;
exports.speechRecognizeWithGCS = speechRecognizeWithGCS;
...
// 型定義を追加
const { SpeechClient } = speech_1.v1p1beta1;
const { RecognitionConfig } = speech_1.protos.google.cloud.speech.v1p1beta1;
...
async function speechRecognize(audioContent, options) {
    const request = {
        audio: {
...
async function speechRecognizeWithGCS(audioContent, options) {
...
function parseResponse(response) {
    ...
    // 話者識別情報を持つ結果を探す
    const diarizedResult = response.results?.find(result => result.alternatives?.[0]?.words?.some(word => word?.speakerTag && word.speakerTag > 0));
    // 通常の文字起こし用の結果を探す
    const transcriptionResult = response.results?.find(result => result.alternatives?.[0]?.transcript);
...
async function uploadGCS(audioContent) {
    // GCSにアップロード
