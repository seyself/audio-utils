...
exports.WavRecorder = void 0;
const audio_processor_js_1 = require("./worklets/audio_processor.js");
const audio_analysis_js_1 = require("./analysis/audio_analysis.js");
const wav_packer_js_1 = require("./wav_packer.js");
/**
...
class WavRecorder {
    ...
    constructor({ sampleRate = 44100, outputToSpeakers = false, debug = false, } = {}) {
        // Script source
        this.scriptSrc = audio_processor_js_1.AudioProcessorSrc;
        // Config
    ...
    static async decode(audioData, sampleRate = 44100, fromSampleRate = -1) {
        ...
        if (audioData instanceof Blob) {
            ...
            if (!data) {
                data = wav_packer_js_1.WavPacker.floatTo16BitPCM(float32Array);
            }
            const audio = {
                bitsPerSample: 16,
            ...
            const packer = new wav_packer_js_1.WavPacker();
    ...
    listenForDeviceChange(callback) {
        if (callback === null && this._deviceChangeCallback) {
            ...
            let lastId = 0;
            let lastDevices = [];
            const serializeDevices = (devices) => devices
                .map((d) => d.deviceId)
                .sort()
                .join(',');
            const cb = async () => {
    ...
    async begin(deviceId) {
        if (this.processor) {
            ...
            console.warn(`Already connected: please call .end() to start a new session`);
            await this.end();
        }
        if (!navigator.mediaDevices ||
        ...
        processor.port.onmessage = (e) => {
            ...
            if (event === 'receipt') {
                ...
                if (this._chunkProcessorSize) {
                    ...
                    this._chunkProcessorBuffer = {
                        raw: wav_packer_js_1.WavPacker.mergeBuffers(buffer.raw, data.raw),
                        mono: wav_packer_js_1.WavPacker.mergeBuffers(buffer.mono, data.mono),
                    };
                    if (this._chunkProcessorBuffer.mono.byteLength >=
    ...
    async pause() {
        if (!this.processor) {
            console.warn('Session ended: please call .begin() first');
            return false;
        }
        else if (!this.recording) {
            console.warn('Already paused: please call .record() first');
            return false;
        }
    ...
    async record(chunkProcessor, chunkSize = 8192) {
        if (!this.processor) {
            throw new Error('セッションが終了しました: 最初に .begin() を呼び出してください');
        }
        else if (this.recording) {
            throw new Error('すでに記録中です: 最初に .pause() を呼び出してください');
        }
        else if (typeof chunkProcessor !== 'function') {
            throw new Error(`chunkProcessorは関数である必要があります`);
        }
