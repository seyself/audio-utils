...
export async function loadAndWriteWavToPushStream(url: string, pushStream: speechsdk.PushAudioInputStream) {
  const response = await fetch(url);
  const arrayBuffer = await response.arrayBuffer();
  writeWavToPushStream(arrayBuffer, pushStream);
}

export function writeWavToPushStream(arrayBuffer: ArrayBuffer, pushStream: speechsdk.PushAudioInputStream) {
...
function getString(dataView: DataView, start: number, length: number) {
  let str = '';
