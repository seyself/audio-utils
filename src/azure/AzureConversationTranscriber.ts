/**
 * @example <code>
 * const stt = useRef<AzureConversationTranscriber>(new AzureConversationTranscriber({
...
interface AzureConversationTranscriberOptions {
...
export default class AzureConversationTranscriber {
  ...
  private _systemText: string;
  private _isRecording: boolean;
  private _audioConfig: speechsdk.AudioConfig | null;
  private _audioContext: AudioContext | null;
  private _analyser: AnalyserNode | null;
  private _mediaStream: MediaStream | null;
  private _pushStream: speechsdk.PushAudioInputStream | null;
  private _recordingFloat32Array: Float32Array;
  private _recordingInt16Array: Int16Array;

  private _workletNode: AudioWorkletNode | null = null;
  private _audioSource: MediaStreamAudioSourceNode | null = null;

  public options: AzureConversationTranscriberOptions = {
  ...
  constructor(options: AzureConversationTranscriberOptions & Partial<EventHandlers> = { enableAudioAnalysis: false }) {
    ...
    if (this._isRecording && this._recognizer) {
      try {
        ...
        await new Promise<void>((resolve, reject) => {
          this._recognizer!.stopTranscribingAsync(
            () => {
  ...
  async startRecognition(audioInputDeviceId: string): Promise<void> {
    try {
      ...
      this._recognizer = new speechsdk.ConversationTranscriber(speechConfig, this._audioConfig);
      
      // 初期無音タイムアウト（ミリ秒単位）を設定
      // 音声認識が開始されてから最初の発話が検出されるまでの時間を制御
      this._recognizer.properties.setProperty(
        speechsdk.PropertyId.SpeechServiceConnection_InitialSilenceTimeoutMs, 
        (this.options.initialSilenceTimeoutMs || 10000).toString()
      );
      // サービスがエンドポイントとして無音を検出した後、認識を終了するまでの無音タイムアウト（ミリ秒単位）を設定
      this._recognizer.properties.setProperty(
        speechsdk.PropertyId.SpeechServiceConnection_EndSilenceTimeoutMs, 
        (this.options.endSilenceTimeoutMs || 200).toString()
      );
      // 検出された無音の持続時間（ミリ秒単位）を設定
      // この時間が経過すると、音声認識システムは発話が終了したと判断し、最終的な認識結果を生成
      this._recognizer.properties.setProperty(
        speechsdk.PropertyId.Speech_SegmentationSilenceTimeoutMs, 
        (this.options.segmentationSilenceTimeoutMs || 200).toString()
      );

      this._recognizer.properties.setProperty(
        speechsdk.PropertyId.SpeechServiceConnection_RecoMode, 
        this.options.recoMode || "CONVERSATION"
      );
  ...
  public async loadAndWriteWavToPushStream(url: string): Promise<void> {
    if (!this.options.useInputFileStream) return;
    if (!this._pushStream) return;
    await loadAndWriteWavToPushStream(url, this._pushStream);
  }

  public async writeWavToPushStream(arrayBuffer: ArrayBuffer): Promise<void> {
    if (!this.options.useInputFileStream) return;
    if (!this._pushStream) return;
    writeWavToPushStream(arrayBuffer, this._pushStream);
  }

  private _setupRecognizerCallbacks(): void {
    if (this._recognizer) {
      this._recognizer.canceled = this._handleCanceled.bind(this);
      this._recognizer.transcribing = this._handleRecognizing.bind(this);
      this._recognizer.transcribed = this._handleRecognized.bind(this);
    }
  }

  private async _startContinuousRecognition(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      this._recognizer!.startTranscribingAsync(
        () => {
  ...
  private _handleCanceled(sender: any, event: speechsdk.ConversationTranscriptionCanceledEventArgs): void {
  ...
  private _handleRecognizing(sender: any, event: speechsdk.ConversationTranscriptionEventArgs): void {
  ...
  private _handleRecognized(sender: any, event: speechsdk.ConversationTranscriptionEventArgs): void {
    if (event.result.text) {
      this.updateLatestText(event.result.text);
  ...
  private _calculateVolume(audioData: Float32Array): number {
    let sum = 0;
  ...
  private _recFloat32(pcmFloat32: Float32Array): void
  {
  ...
  private _recInt16(pcmInt16: Int16Array): void
  {
  ...
  private async _startAudioProcessing(): Promise<void> {
    ...
    workletNode.port.onmessage = (event) => {
      if (!this.options.useInputMicStream) return;
      if (!this._pushStream) return;

      const pcmFloat32 = event.data.data as Float32Array;
      ...
      const int16Array: any = float32ToInt16(resampledData);

      if (this.options.debugRecording) this._recFloat32(pcmFloat32);
      if (this.options.debugRecording) this._recInt16(int16Array);
      ...
      if (this.options.useBuffering)
      {
        ...
        if (totalLength > 16000 * (this.options.bufferTimeSec || 0.1)) {
          ...
          const merged: any = new Int16Array(totalLength);
          let offset = 0;
  ...
  private async _setupAudioAnalyser(deviceId: string): Promise<void> {
  ...
  private _startAudioAnalysis(): void {
    ...
    const timeDataArray: Float32Array = new Float32Array(this._analyser.frequencyBinCount);
    const frequencyDataArray: Float32Array = new Float32Array(this._analyser.frequencyBinCount);
 ...
 */
function toMono(float32Array: Float32Array, numChannels: number): Float32Array {
  if (numChannels === 1) {
 ...
 */
function resampleLinear(inputFloat32: Float32Array, originalRate: number, targetRate: number): Float32Array {
...
function convertFloat32ArrayToAudioBuffer(float32Array: Float32Array, audioContext: AudioContext, sampleRate: number, numChannels: number): AudioBuffer {
...
function convertInt16ArrayToAudioBuffer(int16Array: Int16Array, audioContext: AudioContext, sampleRate: number, numChannels: number): AudioBuffer {
